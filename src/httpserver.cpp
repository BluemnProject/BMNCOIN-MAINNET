...
{
    RenameThread("bluemn-httpworker");
    queue->Run();
}
...
void StopHTTPServer()
{
    ...
    if (eventBase) {
        ...
        if (!threadHTTP.try_join_for(boost::chrono::milliseconds(1000))) {
#else
        if (!threadHTTP.timed_join(boost::posix_time::milliseconds(1000))) {
#endif
