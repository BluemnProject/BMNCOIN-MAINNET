// Copyright (c) 2014-2017 The Bluemn Core developers
...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Bluemn related functionality
...
/*
    ...
    allow endless transaction that would bloat Bluemn and make it unusable. To
...
{
    if(fLiteMode) return; // disable all Bluemn specific functionality
